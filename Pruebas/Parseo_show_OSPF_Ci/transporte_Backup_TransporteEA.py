...
def pareso_y_registro_show_ospf(mysql_conn,credenciales_mysql,database,PATH_HC_NE,**kwargs):
    ...
    output_txt = PATH_HC_NE + "/" + nodo + "/output/CISCO_" + nodo + "_ospf.txt"

    # archivo con el template txtfsm para el parseo del output
    template = "/usr/local/tambo/cels/cel_transporte/airflow2/dags/Backup_TransporteEA/templates/parser_show_ospf_cisco.fsm"
...
teco_ansible_runner = PythonOperator(
        ...
        op_kwargs={
            'pbook_dir':'/usr/local/airflow/dags/cel_transporte/Backup_TransporteEA/ansible/',
            'playbook':'transporte_practica_hc_EA.yaml',
...
pareso_y_registro_show_ospf = PythonOperator(
        task_id="pareso_y_registro_show_ospf",
        python_callable=pareso_y_registro_show_ospf,
...
#Secuencia
teco_ansible_runner >> pareso_y_registro_show_ospf